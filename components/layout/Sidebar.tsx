...
function isNavItemActive(href: string, pathname: string, allHrefs: string[]): boolean {
  ...
  return !allHrefs.some(
    (other) =>
      other !== href && other.startsWith(`${href}/`) && (pathname === other || pathname.startsWith(`${other}/`)),
  )
}
...
export function Sidebar({ title, items, footer, badge }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'border-border bg-card fixed inset-y-0 left-0 z-40 flex h-dvh w-64 flex-col overflow-hidden border-r p-5 transition-transform md:static md:h-full md:translate-x-0',
