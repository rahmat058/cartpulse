...
export function AdminOverviewCharts({ initialData }: { initialData: OverviewAnalyticsData }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/analytics?days=${range}`)
      ...
      .then((json: { data: OverviewAnalyticsData }) => setData(json.data))
  ...
  return (
    <section className="space-y-4">
            ...
            <button
              ...
              className={cn(
                ...
                range === days
                  ...
                  : 'border border-border bg-background text-muted-foreground hover:bg-teal-50 hover:text-teal-700 dark:hover:bg-teal-950/40',
              )}
            >
              {days}d
            </button>
      ...
      <div className={cn('grid gap-4 lg:grid-cols-2', loading && 'opacity-60')}>
        <div className="rounded-md border border-teal-100/80 bg-card p-4 shadow-sm dark:border-teal-900/40">
          <div className="mb-3 flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                ${data.totalRevenue.toFixed(2)} in selected period
              </p>
            </div>
          </div>
        ...
        <div className="rounded-md border border-cyan-100/80 bg-card p-4 shadow-sm dark:border-cyan-900/40">
          <div className="mb-3 flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{data.totalOrders} orders in selected period</p>
            </div>
          </div>
