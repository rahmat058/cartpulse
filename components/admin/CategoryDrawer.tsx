...
export function CategoryDrawer({ open, mode, categoryId, defaultParentId, onClose, onSuccess }: CategoryDrawerProps) {
        ...
        <>
          ...
          >
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {mode === 'create'
                      ? 'Organize products with top-level categories and optional subcategories.'
                      : 'Update category details, ordering, and parent assignment.'}
                  </p>
                </div>
                <button
                  type="button"
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              <CategoryForm
                key={mode === 'create' ? `create-category-${defaultParentId ?? 'root'}` : categoryId}
                categoryId={mode === 'edit' ? categoryId : undefined}
                defaultParentId={mode === 'create' ? defaultParentId : undefined}
                variant="drawer"
                onClose={onClose}
                onSuccess={onSuccess}
              />
